...
Every tool's result is size-capped. When a result is cut in the middle (a long test run, a big grep), the full text is saved under the system temp dir (`tiny-agent/output-*.txt`), and the cut notice gives the path so the model can grep or read it instead of running the command again.
